...
<div class="d-flex flex-column gap-4">
    <div class="vd-stat-grid">
        ...
        <div class="vd-stat-card"><div class="vd-stat-label">Upcoming</div><div class="vd-stat-value"><?= count($upcoming) ?></div><div class="vd-stat-sub">Verified appointments</div></div>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if (!$entry['checkin_id'] && $entry['appointment_status'] === 'Confirmed'): ?>
                                    <button type="button" class="btn vd-btn-gold btn-sm" data-check-in="<?= (int) $entry['appointment_id'] ?>">Mark Arrived</button>
    ...
    <div class="vd-dash-card">
        ...
        <div class="vd-dash-card-body">
            <?php if (!$upcoming): ?><div class="vd-empty-state">No upcoming verified appointments.</div>
...
(function () {
    ...
    document.querySelectorAll('[data-check-in]').forEach(button => {
        button.addEventListener('click', async () => {
            if (!confirm('Record this patient as arrived?')) return;
            button.disabled = true;
            const body = new FormData();
            body.append('action', 'checkIn');
            body.append('appointment_id', button.dataset.checkIn);
