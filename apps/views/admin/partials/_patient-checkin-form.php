...
$noKnownConditions = $profileComplete && !$selectedConditions && empty($patient['cond_others']);
...
$generalQuestions = [
    ...
    'drugs'=>'Uses prohibited drugs?', 'allergy'=>'Has allergies?'
];
$womenQuestions = ['pregnant'=>'Pregnant?', 'nursing'=>'Nursing?', 'birth_control'=>'Taking birth-control pills?'];
$alerts = [];
    ...
    <form id="checkinPatientForm" novalidate>
      ...
      <div class="vd-dash-card mb-4" id="checkin-section-personal" data-checkin-section="personal">
        ...
        <div class="vd-dash-card-body"><fieldset data-checkin-fieldset="personal" disabled><div class="row g-3">
          ...
          <div class="col-md-6"><label class="vd-label form-label">Phone Number *</label><input type="tel" class="form-control vd-input" name="phone_number" maxlength="20" value="<?= checkinVal($patient['phone_number']) ?>" required></div>
      ...
      <div class="vd-dash-card mb-4" id="checkin-section-medical" data-checkin-section="medical">
        ...
        <div class="vd-dash-card-body"><fieldset data-checkin-fieldset="medical" disabled>
          <?php foreach ($generalQuestions as $field=>$label): ?><div class="row align-items-center py-2 border-bottom"><div class="col-md-7"><?= htmlspecialchars($label) ?> *</div><div class="col-md-5"><?= checkinRadio($field,'yes',$patient[$field]===null?null:($patient[$field]?'yes':'no')) ?><?= checkinRadio($field,'no',$patient[$field]===null?null:($patient[$field]?'yes':'no')) ?></div></div><?php endforeach; ?>
          <div class="vd-medical-subsection mt-3">For women only</div>
          <?php foreach ($womenQuestions as $field=>$label): ?><div class="row align-items-center py-2 border-bottom"><div class="col-md-7"><?= htmlspecialchars($label) ?></div><div class="col-md-5"><?= checkinRadio($field,'yes',$patient[$field]===null?null:($patient[$field]?'yes':'no')) ?><?= checkinRadio($field,'no',$patient[$field]===null?null:($patient[$field]?'yes':'no')) ?></div></div><?php endforeach; ?>
          <div class="row g-3 mt-2"><?php foreach (['medical_condition_detail'=>'Medical treatment details','serious_illness_detail'=>'Illness/operation details','hospitalized_detail'=>'Hospitalization details','medication_detail'=>'Medication details','allergy_detail'=>'Allergy details'] as $field=>$label): ?><div class="col-md-6"><label class="vd-label form-label"><?= $label ?></label><input class="form-control vd-input" name="<?= $field ?>" value="<?= checkinVal($patient[$field]) ?>"></div><?php endforeach; ?><div class="col-md-3"><label class="vd-label form-label">Blood Type</label><input class="form-control vd-input" name="blood_type" value="<?= checkinVal($patient['blood_type']) ?>"></div><div class="col-md-3"><label class="vd-label form-label">Blood Pressure</label><input class="form-control vd-input" name="blood_pressure" value="<?= checkinVal($patient['blood_pressure']) ?>"></div></div>
...
(function(){
 ...
 const general=<?= json_encode(array_keys($generalQuestions)) ?>, women=<?= json_encode(array_keys($womenQuestions)) ?>;
 function state(){const detailMap={medical_condition:'medical_condition_detail',serious_illness:'serious_illness_detail',hospitalized:'hospitalized_detail',medication:'medication_detail',allergy:'allergy_detail'},detailsComplete=Object.entries(detailMap).every(([question,detail])=>!form.querySelector(`[name="${question}"][value="yes"]:checked`)||Boolean(value(detail)));return {personal:['firstname','lastname','birthdate','gender','phone_number'].every(value),contact:named('contact_confirmed')?.checked===true,dental:Boolean(value('reason_for_visit')),medical:general.every(answered)&&(value('gender')!=='Female'||women.every(answered))&&detailsComplete,conditions:noKnown.checked||conditionInputs().length>0||Boolean(value('cond_others')),consent:Boolean(value('consent_name'))&&Boolean(value('consent_for'))};}
 function refresh(){const s=state();Object.entries(s).forEach(([key,done])=>{const row=document.querySelector(`[data-readiness="${key}"]`),icon=row?.querySelector('i');row?.classList.toggle('is-complete',done);if(icon)icon.className=done?'ti ti-circle-check':'ti ti-circle';});return s;}
 ...
 function issues(s){const rows=[];if(!s.personal)rows.push(['Complete required profile details.','personal']);if(!s.contact)rows.push(['Confirm the phone number and email.','personal']);if(!s.dental)rows.push(['Record the reason for visit.','dental']);if(!s.medical)rows.push(['Answer every required medical question.','medical']);if(!s.conditions)rows.push(['Select conditions or confirm no known conditions.','medical']);if(!s.consent)rows.push(['Complete consent information.','consent']);if(!rows.length)return false;errorBox.innerHTML=`<strong>Complete these items before marking the patient ready:</strong><ul class="mb-0 mt-2">${rows.map(([m,s])=>`<li><button type="button" class="btn btn-link p-0 text-start" data-issue="${s}">${m}</button></li>`).join('')}</ul>`;errorBox.classList.remove('d-none');errorBox.querySelectorAll('[data-issue]').forEach(b=>b.addEventListener('click',()=>{const s=b.dataset.issue;form.querySelector(`[data-checkin-fieldset="${s}"]`).disabled=false;document.getElementById(`checkin-section-${s}`)?.scrollIntoView({behavior:'smooth',block:'start'});}));errorBox.scrollIntoView({behavior:'smooth',block:'center'});return true;}
 ...
 document.getElementById('saveCheckinDraft').addEventListener('click',function(){enableAll();const body=new FormData(form);body.set('save_mode','draft');save(body,this,'Saving…');});
 ...
 form.addEventListener('input',refresh);form.addEventListener('change',refresh);form.querySelectorAll('[name="allergy"],[name="medication"],[name="medical_condition"],[name="serious_illness"],[name="pregnant"],[name$="_detail"]').forEach(i=>i.addEventListener('change',alerts));refresh();
})();
</script>
