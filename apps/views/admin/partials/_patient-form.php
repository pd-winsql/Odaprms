...
<div class="d-flex flex-column gap-4">
    <div class="d-flex justify-content-between align-items-start gap-3">
        <div>
            <div class="vd-welcome-greet">FRONT DESK PATIENT FORM</div>
            <div class="vd-welcome-name"><?= staffVal($patient['full_name']) ?></div>
            <p class="text-muted small mt-2 mb-0">Complete the required profile before marking the patient ready.</p>
        </div>
        ...
        <div class="vd-dash-card mb-4">
            ...
            <div class="vd-dash-card-body">
                ...
                $medicalQuestions = [
                    ...
                    'medical_condition' => 'Under medical treatment?',
                    'serious_illness' => 'Serious illness or operation?',
                    'hospitalized' => 'Previously hospitalized?',
                    'medication' => 'Taking medication?',
                    'smoke' => 'Smokes?',
                    'alcohol' => 'Uses alcohol?',
                    'drugs' => 'Uses prohibited drugs?',
                    'allergy' => 'Has allergies?',
                    'pregnant' => 'Pregnant?',
                    'nursing' => 'Nursing?',
                    'birth_control' => 'Taking birth-control pills?'
                ];
                foreach ($medicalQuestions as $field => $label): ?>
                    <div class="row align-items-center py-2 border-bottom">
                        <div class="col-md-7"><?= htmlspecialchars($label) ?></div>
                        <div class="col-md-5"><?= staffRadio($field, 'yes', $patient[$field] === null ? null : ($patient[$field] ? 'yes' : 'no')) ?><?= staffRadio($field, 'no', $patient[$field] === null ? null : ($patient[$field] ? 'yes' : 'no')) ?></div>
                    </div>
                <?php endforeach; ?>
...
<script>
    (function() {
        ...
        document.getElementById('saveStaffPatientDraft')?.addEventListener('click', async function() {
            const form = document.getElementById('staffPatientForm');
            ...
            try {
                ...
                document.getElementById('staffPatientFormError').textContent = error.message;
                document.getElementById('staffPatientFormError').classList.remove('d-none');
                this.disabled = false;
            }
        });
