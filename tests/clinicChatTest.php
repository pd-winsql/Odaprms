...
$users = [];
try {
    foreach (['Patient', 'Patient', 'Admin', 'Dental Assistant'] as $role) {
    ...
    [$p1, $p2, $admin, $assistant] = array_map(fn($id) => new ChatModel($db, $id), $users);
    ...
    $rows = $admin->inbox('ChatTest', 0)['conversations'];
    ...
    $incoming = $admin->messages($id)['messages'][0];
    $admin->markRead($id, (int) $incoming['message_id']);
