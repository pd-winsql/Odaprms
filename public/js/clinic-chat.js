(() => {
    ...
    const notice =
        "For appointment and clinic inquiries. Replies are available during clinic hours. Not for emergencies.";
    ...
    async function api(action, values = {}, write = false) {
        ...
        if (!response.ok || !data.success) {
            const error = new Error(
                data.message || "Unable to load messages. Try again.",
            );
            error.chatConflict =
    ...
    function mount(root) {
        ...
        const title = node(
            "h2",
            "",
            patient ? "Clinic support" : "Choose a conversation",
        );
        ...
        const threadMeta = node(
            ...
            "vd-chat-thread-meta",
            patient
                ? "Private conversation with the clinic team"
        ...
        const empty = node(
            ...
            "vd-chat-empty",
            patient
                ? "How can we help? Send your appointment or clinic question below."
                : "Select a patient from the inbox to read and reply.",
        );
        log.append(empty);
        const status = node("p", "vd-chat-status");
        status.setAttribute("role", "status");
        const retry = node("button", "vd-chat-retry", "Retry");
        retry.type = "button";
        retry.hidden = true;
        ...
        if (!patient) {
            ...
            const sidebarHeading = node(
                "header",
                "vd-chat-conversations-heading",
            );
            sidebarHeading.append(
                node("h2", "", "Conversations"),
                node("p", "", "Shared patient inbox"),
            );
            ...
            label.append(search);
            list = node("div", "vd-chat-conversation-list");
            list.setAttribute("aria-label", "Patient conversations");
            listStatus = node("p", "vd-chat-status");
            listStatus.setAttribute("role", "status");
            listRetry = node("button", "vd-chat-retry", "Retry loading inbox");
            listRetry.type = "button";
            listRetry.hidden = true;
            listRetry.onclick = inbox;
            const paging = node("div", "vd-chat-paging");
            previous = node("button", "", "Previous");
        ...
        function failure(error) {
            status.textContent =
                error.name === "TimeoutError"
                    ? "The request timed out. Your draft is still here; try again."
                    : error.message;
            retry.hidden = false;
        }
        function remember() {
            ...
            if (input.value)
                drafts.set(key, {
                    body: input.value,
                    baseMessageId: composeBaseMessageId,
                });
        ...
        function rebuildDaySeparators() {
            log.querySelectorAll(".vd-chat-day").forEach((separator) =>
                separator.remove(),
            );
            let previousDay = "";
        ...
        function draw(messages, prepend = false) {
            ...
            for (const m of messages) {
                ...
                seen.add(key);
                first = first ? Math.min(first, key) : key;
                last = Math.max(last, key);
                const outgoing = patient
                    ? m.sender_role === "Patient"
                    : m.sender_role !== "Patient";
                const item = node(
                    "article",
                    `vd-chat-message${outgoing ? " is-mine" : ""}`,
                );
                ...
                const time = node(
                    "time",
                    "",
                    parsedDate(m.created_at).toLocaleTimeString([], {
                        hour: "numeric",
                        minute: "2-digit",
                    }),
                );
            ...
            if (prepend) log.prepend(fragment);
        ...
        async function fetchMessages(before = 0) {
            ...
            do {
                ...
                markReadThrough = Math.max(
                    markReadThrough,
                    Number(data.markReadThrough) || 0,
                );
            ...
            if (active && !document.hidden && markReadThrough) {
                const readState = await api(
                    "read",
                    { conversation_id: id, through: markReadThrough },
                    true,
                );
        ...
        function renderInbox(result) {
            ...
            result.conversations.forEach((c) => {
                const button = node("button", "vd-chat-conversation");
                button.type = "button";
                button.dataset.conversation = c.conversation_id;
                button.setAttribute(
                    "aria-pressed",
                    String(Number(c.conversation_id) === id),
                );
                const row = node("div", "vd-chat-conversation-name");
                row.append(node("strong", "", c.patient_name));
                const activity = node(
                    "span",
                    "vd-chat-conversation-time",
                ...
                if (Number(c.unread))
                    details.append(
                        node("span", "vd-chat-count", String(c.unread)),
                    );
                button.append(row, details);
                button.onclick = () =>
                    select(
                        Number(c.conversation_id),
                        c.patient_name,
                        c.updated_at,
                    );
                list.append(button);
            ...
            if (focusedId)
                list.querySelector(
                    `[data-conversation="${Number(focusedId)}"]`,
                )?.focus();
            previous.disabled = offset === 0;
            next.disabled = !result.hasMore;
        }
        async function inbox() {
            if (patient || disposed || !root.isConnected || document.hidden)
                return;
            if (listBusy) return;
            listBusy = true;
        ...
        function adaptiveDelay(activity) {
            ...
            const base =
                idleCycles <= 2 ? 5000 : idleCycles <= 6 ? 15000 : 30000;
        ...
        async function sync() {
            ...
            if (document.hidden || (patient && !active)) {
                stopSync();
                return;
            }
            if (fetchingMessages || sending || listBusy) {
                scheduleSync(1000);
                return;
            }
            ...
            try {
                ...
                enable();
                if (!disposed && root.isConnected)
                    scheduleSync(catchUp ? 0 : adaptiveDelay(activity));
            }
        }
        ...
        async function select(nextId, name, updatedAt) {
            ...
            last = first = 0;
            seen.clear();
            pendingKey = pendingText = composeBaseMessageId = null;
            ...
            list.querySelectorAll("button").forEach((b) =>
                b.setAttribute(
                    "aria-pressed",
                    String(Number(b.dataset.conversation) === id),
                ),
            );
            await refresh();
            ...
            try {
                ...
                if (e.chatConflict) {
                    try {
                        ...
                        /* The conflict message remains actionable. */
                    }
                    composeBaseMessageId = last;
                    remember();
                ...
                sending = false;
                enable();
                if (active && root.isConnected) input.focus();
            }
        });
        const instance = {
            open() {
