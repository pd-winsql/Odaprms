...
Patients open **Message clinic** from their dashboard. Dental Assistant users open **Messages** in the sidebar; Admin accounts cannot access clinic messages. A patient's first message creates their single ongoing conversation. Read state is shared by the clinic team, and replies identify the dental assistant who sent them. If another assistant replies while a draft is being composed, the draft is preserved while the sender reviews the latest message. Messages do not change appointments or billing records.

The implementation uses the existing PHP/PDO connection and session/CSRF helpers. No new package, queue worker, cron job, WebSocket server, or external messaging provider is needed. Message text supports UTF-8 and is limited to 2,000 characters. Older messages and the inbox are paginated. Open conversations refresh every 5 seconds; the unread badge refreshes every 30 seconds. Hidden browser tabs pause polling.
